...
def _writable_dir(path):
    """True if we can really create a file in this folder (a real test, because
    read-only containers and odd volumes can lie about permissions)."""
    try:
...
# Pick a folder we can write to. If the preferred one (for example a volume
# mounted with the wrong owner, or a read-only container) is not writable, keep
# the site online with a temporary database instead of crashing at start-up,
# and say so in the logs.
if not _writable_dir(DATA_DIR):
    for _fallback in (os.path.join(BASE_DIR, "instance"),
                      os.path.join(tempfile.gettempdir(), "myportfolio")):
        if _writable_dir(_fallback):
            log.warning("%s is not writable - using %s instead. Data will NOT survive a redeploy.",
                        DATA_DIR, _fallback)
            DATA_DIR = _fallback
            DB_PATH = os.path.join(DATA_DIR, os.path.basename(DB_PATH))
            break
    else:
        log.error("No writable folder found for the database; database features will be unavailable.")
...
_proxies = _int_env("TRUST_PROXY", 1)
if _proxies > 0:
...
def ensure_db():
    """Create the tables if needed. Never raises: if the database is not
    usable the site still boots (pages that need no database keep working) and
    the reason is printed in the logs. It is retried on the next request."""
    global _db_ready
    if _db_ready:
        return True
    try:
        init_db()
        _db_ready = True
    except Exception:  # noqa: BLE001 - we want to survive any storage problem
        log.exception("Database is not ready (path: %s)", DB_PATH)
    return _db_ready
...
def init_db():
    ...
    try:
        # WAL lets several workers read while one writes. Some network/virtual
        # disks do not support it, so fall back to SQLite's default mode.
        db.execute("PRAGMA journal_mode=WAL")
        db.execute("PRAGMA synchronous=NORMAL")
    except sqlite3.Error:
        log.warning("WAL mode not supported here; using the default journal mode.")
    db.execute(
        """
