...
# If the data folder cannot be written (for example a volume mounted with the
# wrong owner), keep the site online with a temporary database instead of
# crashing, and say so loudly in the logs.
try:
    os.makedirs(DATA_DIR, exist_ok=True)
    _writable = os.access(DATA_DIR, os.W_OK)
except OSError:
    _writable = False
if not _writable:
    ...
    log.warning("DATABASE_PATH is not writable - using %s instead. Data will NOT survive a redeploy.", DB_PATH)
